...
def run_ingestion(document_id: str) -> None:
    ...
    try:
        ...
        if not doc:
            log.error(f"Document {document_id} not found")
            return

        log.info(f"Starting ingestion for {doc.filename}")

        # Parse document
        from ai.ingestion.router import parse_document
        parsed = parse_document(doc.file_path)
        log.info(f"Parsed {doc.filename} — {parsed.page_count} pages")
        ...
        meta_store.insert_chunks([{
            ...
            "title":       e.chunk.metadata.get("title", doc.filename),
            "file_type":   e.chunk.metadata.get("file_type", doc.file_type),
        ...
        log.info(f"✅ Ingestion completed successfully for {doc.id}")

    except Exception as exc:
        log.exception(f"❌ Ingestion failed for document {document_id}")
        if db and doc:
            doc.status = "failed"
